        ...
        <div className="flex h-screen w-screen bg-[#032044] text-white overflow-hidden font-sans">
            ...
            <aside className="w-64 glass-card m-4 mr-0 flex flex-col border-r-0 rounded-r-none">
                <div className="p-8">
                    ...
                    <p className="text-[10px] uppercase tracking-[0.2em] text-white/20 font-bold mt-1">Student Command Center</p>
                </div>
                ...
                <div className="p-4 mt-auto">
                    <div className="bg-white/5 rounded-2xl p-4 border border-white/5">
                        <div className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-full bg-utsa-orange flex items-center justify-center font-bold">JS</div>
                            <div>
                        ...
                        {activeTab === 'explore' && (
                            <div className="max-w-4xl mx-auto space-y-8">
                                <div className="flex items-end justify-between">
                                    <div>
                                        <h2 className="text-3xl font-black text-white">Course Explorer</h2>
                                        <p className="text-white/40 text-sm mt-1">Search through 4,000+ classes with live RMP and Grade data.</p>
                                    </div>
                                    <div className="flex gap-2">
                                        {['Undergraduate', 'Face-to-Face', 'No Conflicts'].map(filter => (
                                            <span key={filter} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-[10px] font-bold text-white/60">
                                                {filter}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                {searchQuery.length > 0 ? (
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                        ...
                                        {filteredCourses.length > 0 && (
                                            <div className="glass-card p-6 flex flex-col items-center justify-center text-center space-y-4 border-dashed opacity-50">
                                                ...
                                                <p className="text-sm font-medium">Select more classes to compare<br />workload and instructor quality.</p>
                                            </div>
                                        )}
                                    </div>
                                ) : (
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-10">
                                        {[
                                            { icon: LayoutDashboard, label: 'Trend Analysis', desc: 'See which sections are filling up fast this term.' },
                                            { icon: User, label: 'Top Rated', desc: 'Faculty with 4.8+ ratings for your major.' },
                                            { icon: Database, label: 'Grade History', desc: 'Courses with historical 40%+ A-Grade distributions.' },
                                        ].map((box, i) => (
                                            <div key={i} className="glass-card p-8 space-y-4 hover:bg-white/5 transition-colors cursor-pointer group">
                                                ...
                                                <p className="text-xs text-white/40 leading-relaxed">{box.desc}</p>
                                            </div>
                                        ))}
                                    </div>
                                )}
                        ...
                        {activeTab === 'schedule' && (
                            <div className="flex flex-col items-center justify-center h-full text-center space-y-6 opacity-40">
                                ...
                                <div className="space-y-2">
                                    <h2 className="text-2xl font-black">Desktop Schedule Visualizer</h2>
                                    <p className="text-sm">The full-screen weekly calendar is currently in the forge.</p>
                                </div>
                            </div>
                        )}
                    </div>
